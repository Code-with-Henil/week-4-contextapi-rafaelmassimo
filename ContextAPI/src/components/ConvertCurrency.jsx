...
function ConvertCurrency() {
	...
	const { setCurrentCurrency, setAmount, setColorTheme } =
	...
	let currentCurrencyInput;
	let currentAmount;
	const usDolarValue = 4.87;
	const cadDolarValue = 3.6;

	useEffect(() => {
		currentCurrencyInput = currentCurrency;
	...
	function convertAmount() {
		...
		if (currentCurrencyInput === "US-dolar") {
			setAmoutConverted(currentAmount / usDolarValue);
		} else if (currentCurrencyInput === "CAD-dolar") {
			setAmoutConverted(currentAmount / cadDolarValue);
		} else {
	...
	return (
		<>
			...
			<div className="p-48 transition duration-1000 ease-in-out flex flex-col w-3/5">
				<form className="flex flex-col" action={convertAmount}>
					<p className="text-3xl font-bold font-normal">
						From Reais Brazil:{' '}{amount.toLocaleString(undefined, {
                            style: "currency",
                            currency: "BRL"
                        })}
					</p>
					<br />
					{currentCurrency !== "" && <h2 className="text-3xl font-semibold m-4">
						To {currentCurrency}:{' '}
					...
					<label htmlFor="amount">Insert here the amount in R$ to be converted</label>
					<input
